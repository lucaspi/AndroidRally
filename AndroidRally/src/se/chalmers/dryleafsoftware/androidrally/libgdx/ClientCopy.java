...
/**
 * This class talks to the server. It converts data from the server to
 * appropriate classes which can then be fetched through getters. It also
 * converts data is should sent to a format the server can read.
 * 
 * @author
 * 
 */
public class ClientCopy {
	...
	// Placeholder for when server is implemented
	@SuppressWarnings("unused")
	private final int clientID; // TODO: the client must be assigned a unique ID
								// from server
	private int robotID; // TODO: get robot ID from server
	private int roundID = 0;
	private static ClientCopy instance;
	 ...
	 */
	public static synchronized ClientCopy getInstance() {
		if (instance == null) {
			instance = new ClientCopy();
		}
		return instance;
	}
	...
	public void loadGame(int gameID) {
		resetGameValues();
		this.controller = new GameController(IOHandler.load(gameID,
				IOHandler.SERVER_DATA));
		controller.newRound();
	}

	/**
	 * Sends the cards to the server. Note: This list should not contain more
	 * then five!
	 * 
	 * @param cards
	 *            The cards to send.
	...
	public RoundResult getRoundResult() {
		...
		Texture damageAnim = new Texture(
				Gdx.files.internal("textures/special/damageAnim.png"));
		damageAnim.setFilter(TextureFilter.Linear, TextureFilter.Linear);
		Texture explodeAnim = new Texture(
				Gdx.files.internal("textures/special/explodeAnim.png"));
		damageAnim.setFilter(TextureFilter.Linear, TextureFilter.Linear);
		...
		for (String s : allActions) {
			...
			if (parallel[0].equals("R")) { // R = card
				...
				if (phase == GameAction.PHASE_LASER) {
					...
					for (int i = 1; i < parallel.length; i++) {
						...
						if (data[1].substring(0, 1).equals("1")) {
							...
							multiExplode.add(new ExplodeAction(Integer
									.parseInt(data[0]), explodeAnim));
						}
					}
					multiDamage.setDuration(1000);
					multiDamage.setPhase(GameAction.PHASE_LASER);
					...
					for (int i = 1; i < parallel.length; i++) {
						String[] data = parallel[i].split(":");
						result.addAction(new CheckPointAction(Integer
								.parseInt(data[0]), Integer.parseInt(data[1]),
								false));
					}
				...
				if (Integer.parseInt(parallel[1]) == robotID) {
					result.addAction(new HolderAction(0,
							HolderAction.SPECIAL_PHASE_GAMEOVER));
				}
			} else if (parallel[0].equals("W")) { // When a player has won.
				result.addAction(new CheckPointAction(Integer
						.parseInt(parallel[1]), CheckPointAction.UNCHANGED,
						true));
				result.addAction(new HolderAction(0,
						HolderAction.SPECIAL_PHASE_WON));
			}
			// Generic multiaction
	...
	private SingleAction createSingleAction(String indata) {
		...
		return new SingleAction(Integer.parseInt(data[0]),
	...
	/**
	 * Gives the robots in the game.
	 * 
	 * @param texture
	 *            The texture to use when creating the robots.
	 ...
	 * 
	 * @param texture
	 *            The textures to use when displaying the robots.
	 * @param dockPositions
	 *            All the docks' positions.
	 * @return A list of all the robots.
	 */
	public List<RobotView> getRobots(Texture texture, Vector2[] dockPositions) {
		...
		for (int i = 0; i < Integer.parseInt(controller.getNbrOfRobots()); i++) {
			RobotView robot = new RobotView(i, new TextureRegion(texture,
					i * 64, 448, 64, 64), new LaserView(new TextureRegion(
					texture, 64 * i, 384, 64, 64), 0), "Player " + (i + 1));
	...
	/**
	 * Gives the ID of the client's robot.
	 * 
	 * @return The ID of the client's robot.
	 */
